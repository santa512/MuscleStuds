...
export default async function CatalogsPage() {
  ...
  return (
    <div>
      <h2 className="text-xl text-center py-5 uppercase">Catalogs</h2>
        ...
        <Link key={catalog.id} href={`/catalogs/${catalog.slug}`}>
          <div className="my-8 flex flex-col md:flex-row gap-4 border-b md:border-none pb-4">
            ...
            <div className="flex flex-col ">
              ...
              <span className="text-sm font-extralight py-2">{convertDate(catalog.date_created)} {catalog.videos?.length > 0 ? `, ${catalog.videos.length} videos` : ''}</span>
